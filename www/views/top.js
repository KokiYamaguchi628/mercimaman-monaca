...
async function get_banner() {
    ...
    try {
        let bannerRes = await app.api.call(appConst.apiUrl.slide.index.method, appConst.apiUrl.slide.index.url, data)
        // console.log(bannerRes.data)
        this.banner = bannerRes ? bannerRes.data : null
        console.log('Koki Yamaguchi', this.banner)
    } catch(err) {
        throw error
    }
...
async function get_slides(){
	...
	try {
		...
		if(res.data.data.length > 0){
            // await console.log('スライダー' + res);

			//画像の入れ込み
			app.shared.top = {slide_list:res.data.data}
            console.log(app.shared.top + 'koki');
...
function createSlide(slide_list){
	...
	for(i = 0; i < slide_list.length; i++){
        // console.log('loop' + i + JSON.stringify(slide_list[i].images[1].url));
        //google モバイルアプリ誘導バナーをスキップ
        if(i == 1) continue;
